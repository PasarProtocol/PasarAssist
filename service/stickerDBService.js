...
module.exports = {
    ...
    listTrans: async function(pageNum, pageSize) {
        ...
        try {
            ...
            let projectFirst = {'_id': 0, event: 1, tHash: 1, from: "$order.sellerAddr", to: "$order.buyerAddr",
             timestamp: "$order.updateTime", price: "$order.price", tokenId: "$order.tokenId", blockNumber: 1};
            let projectSecond = {'_id': 0, event: "notSetYet", tHash: "$txHash", from: 1, to: "$order.buyerAddr",
             timestamp: 1, price: "$memo", tokenId: 1, blockNumber: 1};
            let projectFinal = {event: 1, tHash: 1, from: 1, to: 1, timestamp: 1, price: 1, tokenId: 1, blockNumber: 1, name: "$token.name", royalties: "$token.royalties"};
                ...
                { $facet: {
                  ...
                  "collection2": [
                    { $lookup: {
                      ...
                      pipeline: [{ $project: {'_id': 0, event: "notSetYet", tHash: "$txHash", from: 1, to: "$order.buyerAddr",
                        timestamp: 1, price: "$memo", tokenId: 1, blockNumber: 1} }],
                      "as": "collection2"
                    }}
                  ]
            ...
            for(var i = 0; i < result.length; i++) {
                if(result[i]['event'] == "notSetYet") {
                    ...
                    if(result[i]['from'] != '0x0000000000000000000000000000000000000000' && result[i]['to'] != '0x0000000000000000000000000000000000000000') {
                        if(result[i]['price'] == '')
                            result[i]['event'] = 'SafeTranserFrom';
                        else result[i]['event'] = 'SafeTranserFromWithMemo';
                    }
                    result[i]['price'] = '0';
                }
